 ...
 * Col 2 — fromBankCode  (String — BIC of sending bank)
 * Col 3 — toBankCode    (String — BIC of receiving bank)
 * Col 4 — nostroAccount (String — correspondent account for settlement)
 * Col 5 — swiftRef      (String — unique SWIFT message reference, mandatory)
 */
public class SwiftAdapter implements TransactionAdapter {

    private static final String SOURCE = SourceType.SWIFT.name();

    private final SourceSystem sourceSystem;

    public SwiftAdapter(SourceSystem sourceSystem) {
    ...
    public IncomingTransaction adapt(Row row, int rowNumber) throws AdapterException {
        try {
            ...
            AdapterUtil.requireCell(row.getCell(4), SOURCE, "nostroAccount", rowNumber);
            AdapterUtil.requireCell(row.getCell(5), SOURCE, "swiftRef",      rowNumber);
