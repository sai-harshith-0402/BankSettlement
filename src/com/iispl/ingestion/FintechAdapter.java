 ...
 * Col 5 — fintechRef    (String — partner's own transaction reference, mandatory)
 * Col 6 — partnerId     (String — identifies which Fintech partner, mandatory)
 */
public class FintechAdapter implements TransactionAdapter {

    private static final String SOURCE = SourceType.FINTECH.name();

    private final SourceSystem sourceSystem;

    public FintechAdapter(SourceSystem sourceSystem) {
    ...
    public IncomingTransaction adapt(Row row, int rowNumber) throws AdapterException {
        try {
            ...
            AdapterUtil.requireCell(row.getCell(5), SOURCE, "fintechRef",    rowNumber);
            AdapterUtil.requireCell(row.getCell(6), SOURCE, "partnerId",     rowNumber);
