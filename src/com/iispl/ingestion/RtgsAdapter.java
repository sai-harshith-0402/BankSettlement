...
/**
 * RtgsAdapter — RTGS ingestion adapter.
...
public class RtgsAdapter implements TransactionAdapter {
    ...
    public List<IncomingTransaction> ingest(String filePath) throws AdapterException {
        ...
        if (!file.exists()) {
            throw new AdapterException(
                    "RTGS .xlsx file not found: " + filePath, getSourceSystemType());
        }
        ...
        try (FileInputStream fis = new FileInputStream(file);
            ...
            if (sheet == null) {
                throw new AdapterException(
                        "RTGS .xlsx has no sheets: " + filePath, getSourceSystemType());
            }

            Row headerRow = sheet.getRow(0);
            if (headerRow == null) {
                throw new AdapterException(
                        "RTGS .xlsx has no header row: " + filePath, getSourceSystemType());
            }
            ...
            System.out.println("[RtgsAdapter] Parsing " + totalRows
                    + " data rows from: " + filePath);

            for (int rowIdx = 1; rowIdx <= totalRows; rowIdx++) {
                Row row = sheet.getRow(rowIdx);
                if (row == null || isRowBlank(row)) {
                    System.out.println("[RtgsAdapter] Skipping blank row at index " + rowIdx);
                    continue;
                }
                try {
                    ...
                    System.err.println("[RtgsAdapter] Skipping row " + rowIdx
                            + " — " + ae.getMessage());
                }
            }

            System.out.println("[RtgsAdapter] Successfully ingested "
            ...
            throw new AdapterException(
                    "RTGS .xlsx ingestion failed for [" + filePath + "]: " + e.getMessage(),
    ...
    public IncomingTransaction adapt(Map<String, String> row) throws AdapterException {
        ...
        try {
            ...
            SourceSystem sourceSystem = new SourceSystem(sourceSystemId, SourceType.RTGS, "");
            ...
            throw new AdapterException(
                    "RTGS adapt() failed: " + e.getMessage(), getSourceSystemType(), e);
        }
    }
    ...
    public Map<TransactionType, List<IncomingTransaction>> segregate(
        ...
        System.out.println("[RtgsAdapter] Segregation — "
    ...
    private void logExtraColumns(Map<Integer, String> headerMap, String filePath) {
        for (String header : headerMap.values()) {
            if (!ALLOWED_COLUMNS.contains(header)) {
                System.out.println("[RtgsAdapter] Extra column ignored: '"
                        + header + "' in " + filePath);
            }
        }
    }
    ...
    private String requireValue(Map<String, String> row, String key) throws AdapterException {
        ...
        if (value == null || value.isBlank()) {
            throw new AdapterException(
                    "Missing required field '" + key + "' in RTGS row", getSourceSystemType());
        }
        return value;
    }
